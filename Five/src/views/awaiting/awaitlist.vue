...
export default {
  ...
  methods: {
    ...
    getdata() {
      this.studentDetail().then(res => {
        if (res.code === 1) {
          if (this.$route.query.id) {
            const data = res.exam.filter(item => item.grade_id === this.$route.query.id)
            if (data.length) {
              this.list = data
            }
          }
    ...
    pijuan() {
      console.log(111)
      this.$router.push(`/markingmanagement/markpapers`)
    }
  }
